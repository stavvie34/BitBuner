...
for (i = 0; i < eligibleServers.length; i = i + 1) {
	...
	tprint('This is the currentServer...');
	tprint(currentServer);
	
	if (i === eligibleServers.length) {
		tprint('Ignoring next server, because current server is the last in the list.');
	} else {
		nextServer = eligibleServers[i + 1];
		tprint('This is the next server...');
		tprint(nextServer);
	}
	
	if (i === 0) {
		tprint('There is no previous server.');
	} else {
		previousServer = eligibleServers[i - 1];
		tprint('This is the previous server...');
		tprint(previousServer);
	}

	
	currentOptimalServer = optimalServers[i];
	tprint('This is the currentOptimalServer...');
	tprint(currentOptimalServer);
	...
	if (i === 0) {
		tprint('Ignoring previous server, because current server is the first in the list.');
	} else {
