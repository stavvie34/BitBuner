...
for (i = 0; i < SvrOnePort.length; i = i + 1) {
	...
	if ((isRunning('hack-target.script', 'home', 25198, targetOnePort)) === false) {
		exec('hack-target.script', 'home', 25198, targetOnePort);
	}
	
	while ((getServerMoneyAvailable(targetOnePort)) > 0) {
	    print((getServerMoneyAvailable(targetOnePort)));
	    sleep(5000);
	}
